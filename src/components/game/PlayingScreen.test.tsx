...
describe('PlayingScreen Component', () => {
  ...
  it('renders correctly with initial props for single-visual mode', () => {
    ...
    const visualGrid = screen.getByText('Position Match').closest('.space-y-8')!.querySelector('.grid.grid-cols-3');
    ...
    expect(screen.getByText(/Press when current stimulus matches 2 steps back/i)).toBeInTheDocument();
  });
  ...
  it('renders correctly for dual mode', () => {
    render(<PlayingScreen {...defaultProps} gameMode="dual" currentPosition={1} currentLetter="C" />);
