...
interface GameSetupScreenProps {
  ...
  nLevel: number;
  setNLevel: (updater: number | ((prev: number) => number)) => void;
  numTrials: number;
  setNumTrials: (updater: number | ((prev: number) => number)) => void;
  stimulusDurationMs: number;
  setStimulusDurationMs: (updater: number | ((prev: number) => number)) => void;
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 p-4">
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <Card className="shadow-lg">
            ...
            <CardContent className="space-y-4">
              {[
                ...
                <div
                  key={m}
                  className={`p-4 border-2 rounded-lg transition-all ${gameMode === m ? "border-blue-500 bg-blue-50" : "border-gray-200"} ${isPracticeMode ? "cursor-not-allowed opacity-70" : "cursor-pointer hover:border-gray-300"}`}
                  onClick={() => {
                    if (!isPracticeMode) setGameMode(m);
                  }}
                >
          ...
          <Card className="shadow-lg">
            ...
            <CardContent className="space-y-6">
              <div>
                <label className="block text-sm font-medium mb-2">N-Level</label>
              ...
              {!isPracticeMode && (
                <div className="pt-4">
                  ...
                  <p className="text-xs text-gray-500 mt-1 pl-1">
                    N-Level adjusts based on performance.
                  </p>
                </div>
              )}
            </CardContent>
        ...
        <div className="flex justify-center mt-8">
          ...
          >
            <Play className="h-5 w-5" /> Start Training Session
          </Button>
        </div>
      </div>
    </div>
