...
describe('ResultsScreen Component', () => {
  ...
  it('renders correctly with lastSession data', () => {
    ...
    expect(screen.getByText((content) => content.startsWith('Actual Visual Matches:') && content.includes('5'))).toBeInTheDocument();
    expect(screen.getByText('Detailed Audio Performance')).toBeInTheDocument();
    expect(screen.getByText((content) => content.startsWith('Actual Audio Matches:') && content.includes('6'))).toBeInTheDocument();
  });
  ...
  it('handles localStorage parsing error gracefully', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith("Failed to parse sessions from localStorage", expect.any(Error));
  ...
  it('renders specific stats for single-visual mode', () => {
    const visualSession = { ...sampleSession, mode: 'single-visual', audioAccuracy: 0, actualAudioMatches: 0, audioHits:0, audioMisses:0, audioFalseAlarms:0, audioCorrectRejections:0 };
  ...
  it('renders specific stats for single-audio mode', () => {
    const audioSession = { ...sampleSession, mode: 'single-audio', visualAccuracy: 0, actualVisualMatches: 0, visualHits:0, visualMisses:0, visualFalseAlarms:0, visualCorrectRejections:0 };
